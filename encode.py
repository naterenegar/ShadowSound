...
outdata = []
def notch(fft, freq):
    b = freq_to_index(max(freq-delta, 0), frames, source.getframerate())
    e = freq_to_index(min(freq+delta, 20000), frames, source.getframerate())
    bm = mirror(b, fft)
    em = mirror(e, fft)
...
for v in toencode:
    print(len(indata))
    toParse = indata[:sample*2]
    indata = indata[sample*2:]
